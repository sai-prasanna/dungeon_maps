...
def draw_categorical_map(topdown_map, mask):
    ...
    colors = np.asarray(CLASS_COLORS, dtype=np.uint8)
    color_map = colors[topdown_map.astype(np.int32)]  # (3, h, w)
    #color_map[~mask] = CLASS_COLORS[0]
    return color_map[0]
...
def draw_origin(
    ...
    pos = np.array(
        [
            ...
            [0.0, 0.0, -1],  # backward vector
            [-1, 0.0, 0.0],  # left-back vector
            [1.0, 0.0, 0.0],  # right-back vector
        ],
        dtype=np.float32,
    )
...
def draw_camera(
    ...
    pos = np.array(
        [
            [0.0, 0.0, 0.0],  # camera position
            [0.0, 0.0, 1.0],  # forward vector
            [-1, 0.0, -1],  # left-back vector
            [1.0, 0.0, -1],  # right-back vector
        ],
        dtype=np.float32,
    )
