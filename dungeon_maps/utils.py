...
_ScatterMethods: Dict[Reduction, Callable] = {
    ...
    Reduction.replace: scatter_replace,
}
...
def get_camera_intrinsics(
    width: float, height: float, hfov: float, vfov: Optional[float] = None
) -> CameraIntrinsics:
...
# ======== PyTorch utils =======
def to_numpy(inputs: Any, dtype: Optional[np.dtype] = None) -> np.ndarray:
...
def to_tensor_like(inputs: Any, tensor: torch.Tensor) -> torch.Tensor:
...
def validate_tensors(
    *args: Any,
    ...
    tensors = []
    for arg in args:
        tensors.append(to_tensor(arg, device=same_device, dtype=same_dtype))
    if len(tensors) == 1 and not keep_tuple:
        return tensors[0]
    return tuple(tensors)


def translate(points: torch.Tensor, offsets: torch.Tensor) -> torch.Tensor:
    ...
    points, offsets = validate_tensors(
        points, offsets, same_device=True, same_dtype=torch.float32
    )
    offsets = offsets.view(-1, 1, 3)  # (b, 1, 3)
    batch = points.shape[0]
    # apply translation
    points = torch.reshape(points.view(batch, -1, 3) + offsets, points.shape)
    return points
...
def rotate(
    ...
    points, axis, angle = validate_tensors(
        points, axis, angle, same_device=True, same_dtype=torch.float32
    )
    ...
    S_flat = torch.stack(
        (zeros, -ax_z, ax_y, ax_z, zeros, -ax_x, -ax_y, ax_x, zeros), dim=-1
    )  # (b, 9)
    S = S_flat.view(-1, 3, 3)
...
def ravel_index(
    index: torch.Tensor, shape: torch.Size, keepdim: bool = False
) -> torch.Tensor:
...
def _get_scatter_method(reduction: Union[str, Reduction]) -> Callable:
...
def scatter_tensor(
    ...
    scatter_method(src=flat_values, index=flat_indices, dim=-1, out=flat_canvas)
...
def to_4D_image(image: torch.Tensor) -> torch.Tensor:
    ...
    assert ndims in [2, 3, 4], f"only supports 2/3/4D images while {ndims}-D are given."
    if ndims == 2:
...
def generate_image_coords(
    ...
    if ndims < 2:
        raise ValueError("rank of `image_shape` must be at east 2D, " f"got {ndims}")
    h = image_shape[-2]
    w = image_shape[-1]
...
def generate_crop_grid(
    ...
    x, y = generate_image_coords(
        (batch, crop_height, crop_width), dtype=torch.float32, device=center.device
    )
    ndims = len(x.shape)
    center_x = (center[..., 0] - w / 2.0).view((-1,) + (1,) * (ndims - 1))  # (b, ...)
    center_y = (center[..., 1] - h / 2.0).view((-1,) + (1,) * (ndims - 1))  # (b, ...)
    x = (x - crop_width / 2.0 + center_x) / (w / 2.0)
    y = (y - crop_height / 2.0 + center_y) / (h / 2.0)
