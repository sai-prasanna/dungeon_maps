...
from dungeon_maps.utils import NINF, CameraIntrinsics, Float3D, Reduction
...
def _mask_borders(masks: torch.Tensor, clip_border: int) -> torch.Tensor:
...
def orth_project(
    ...
    if (clip_border is not None) and (clip_border > 0):
        valid_area = _mask_borders(masks=valid_area, clip_border=clip_border)
    ...
    if to_global:
        point_cloud = local_to_global_space(
            points=point_cloud, cam_pose=cam_pose, _validate_args=False
        )
    # Flatten point cloud
    # (b, ..., h, w, 3) -> (b, ..., h*w, 3)
...
def camera_affine_grid(
    ...
    if _validate_args:
        ...
        (depth_map, trans_pose, cam_pitch, cam_height) = utils.validate_tensors(
            depth_map, trans_pose, cam_pitch, cam_height, same_device=device or True
        )
        # Ensure tensor shape at least 4D (b, ..., h, w)
        depth_map = utils.to_4D_image(depth_map)  # (b, c, h, w)
    ...
    # Apply transition
    point_cloud = local_to_global_space(
        points=point_cloud, cam_pose=trans_pose, _validate_args=False
    )
...
def depth_map_to_point_cloud(
    ...
    x, y = utils.generate_image_coords(
        depth_map.shape, dtype=torch.float32, device=device
...
def height_map_to_point_cloud(
    ...
    if _validate_args:
        ...
        (height_map, width_offset, height_offset) = utils.validate_tensors(
            height_map, width_offset, height_offset, same_device=device or True
        )
    ...
    x_bin, z_bin = utils.generate_image_coords(
        height_map.shape, dtype=torch.float32, device=device
    )
    x, z = map_dequantize(
        x_coords=x_bin,
        z_coords=z_bin,
...
def image_to_camera_space(
    ...
    if flip_h and height is None:
        if len(points.shape) < 3:
            raise RuntimeError(
                "The rank of `points` must be at least 3D (..., h, w, 3) "
...
def camera_to_local_space(
    ...
    points = utils.rotate(points, [1.0, 0.0, 0.0], cam_pitch)
...
def local_to_global_space(
    ...
    points = utils.rotate(points, [0.0, 1.0, 0.0] * b, yaw)
...
def global_to_local_space(
    ...
    b = 1 if len(points.shape) < 2 else points.shape[0]
    points = utils.rotate(points, [0.0, 1.0, 0.0] * batch, -yaw)
    if _validate_args:
        points = points.view(orig_shape)
    return points
...
def map_quantize(
    ...
    if _validate_args:
        (x_coords, z_coords, width_offset, height_offset) = utils.validate_tensors(
            x_coords, z_coords, width_offset, height_offset, same_device=device or True
        )
        # Ensure tensor shapes
...
def project(
    ...
    if _validate_args:
        (coords, values, masks, canvas, canvas_masks) = utils.validate_tensors(
            coords, values, masks, canvas, canvas_masks, same_device=device or True
        )
    ...
    low_inds = 0
    up_inds = torch.tensor(canvas.shape[-2:], dtype=torch.int64, device=device)
    # Filtering invalid area (b, ..., n)
    valid_area = torch.cat(
        (indices >= low_inds, indices < up_inds, masks.unsqueeze(dim=-1)), dim=-1
    ).all(dim=-1)
...
def compute_center_offsets(
    ...
    if center_mode is CenterMode.none:
        ...
        if center_mode is CenterMode.camera:
            if to_global:
                cam_pos = local_to_global_space(
                    points=center_pos, cam_pose=cam_pose, _validate_args=True
                )
                center_pos = cam_pos
        ...
        w_offset = map_width / 2.0 - center_pos_x
        h_offset = map_height / 2.0 - center_pos_z
...
class MapProjector:
    def __init__(
        ...
        self.cam_params: CameraIntrinsics = utils.get_camera_intrinsics(
            width=self.width, height=self.height, hfov=self.hfov, vfov=self.vfov
        )
...
class TopdownMap:
    ...
    def __init__(
        ...
        if is_height_map is None:
            is_height_map = not self.is_empty and topdown_map is height_map
    ...
    def get_camera(self) -> torch.Tensor:
        ...
        cam_pos = self.get_coords(points=cam_pos, is_global=False)  # (b, 1, 2)
    ...
    def get_origin(
        ...
        origin_pos = self.get_coords(points=origin_pos, is_global=True)  # (b, 1, 2)
        return origin_pos.squeeze(dim=-2)  # (b, 2)

    def get_coords(self, points: torch.Tensor, is_global: bool = True) -> torch.Tensor:
        ...
        if self.proj.to_global:
            if not is_global:
                ...
                points = self.proj.local_to_global_space(points=points)
        else:
            if is_global:
                # Convert to local coordinates
                points = self.proj.global_to_local_space(points=points)
        # Convert to image coordinates
        pos_x, pos_z = self.proj.map_quantize(
            x_coords=points[..., 0], z_coords=points[..., 2]
        )
        pos = torch.stack((pos_x, pos_z), dim=-1)  # (b, n, 2)
        return pos

    def get_points(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        pos_x, pos_z = self.proj.map_dequantize(
            x_coords=coords[..., 0], z_coords=coords[..., 1]
        )
        pos = torch.stack((pos_x, pos_z), dim=-1)  # (b, n, 2)
        return pos
...
def crop_topdown_map(
    ...
    if _validate_args:
        (center, width_offset, height_offset) = utils.validate_tensors(
            center, proj.width_offset, proj.height_offset, same_device=True
        )
    ...
    height_map = utils.image_sample(
        image=source.height_map, grid=grid, fill_value=NINF, mode=mode
    )
    # crop masks
    mask = utils.image_sample(image=source.mask, grid=grid, fill_value=False, mode=mode)
    # crop top-down map
...
def _merge_point_clouds(
    *maps: List[TopdownMap], map_projector: MapProjector
) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
    ...
    for index, _map in enumerate(maps):
        if not _map.is_empty:
            (_map_points, _map_masks, _map_values) = _flattened_topdown_map(_map)
    ...
    points = utils.validate_tensors(
        *points, same_device=True, same_dtype=torch.float32, keep_tuple=True
    )
    masks = utils.validate_tensors(
        *masks, same_device=points[0].device, same_dtype=torch.bool, keep_tuple=True
    )
    # (b, ..., h*w, 3)
...
def _compute_bounding_box(
    x_coords: torch.Tensor, z_coords: torch.Tensor, keepdim: bool = False
...
def _compute_new_shape_and_offsets(
    points: torch.Tensor, map_projector: MapProjector
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    min_x, max_x, min_z, max_z = _compute_bounding_box(
        x_coords=x_coords, z_coords=z_coords
    )  # (b,)
    ...
    # Compute offsets
    center_pos_x = (max_x + min_x) / 2.0
    center_pos_z = (max_z + min_z) / 2.0
    width_offset = map_width / 2.0 - center_pos_x
    height_offset = map_height / 2.0 - center_pos_z
...
def fuse_topdown_maps(
    *maps: List[TopdownMap],
    ...
    proj = map_projector
    # merge point clouds
    points, masks, values = _merge_point_clouds(*maps, map_projector=map_projector)
    if points is None:
        # Return empty topdown map
        return TopdownMap(map_projector=map_projector)
    ...
    (
        ...
        width_offset,
        height_offset,
    ) = _compute_new_shape_and_offsets(points=points[masks], map_projector=proj)
...
class MapBuilder:
    def __init__(
        ...
        if self._world_map is None:
            ...
            self._world_map = TopdownMap(map_projector=self.proj.clone())
    ...
    def step(
        ...
        **kwargs: Dict[str, Any],
        ...
        if merge:
            self.merge(topdown_map, keep_pose=keep_pose)
        return topdown_map
    ...
    def plot(
        ...
        **kwargs: Dict[str, Any],
        ...
        cam_pose = get(
            cam_pose, self.proj.cam_pose, np.array([0.0, 0.0, 0.0], dtype=np.float32)
        )
        # compute offsets
        width_offset, height_offset = self._compute_offsets(
            cam_pose=cam_pose, center_mode=center_mode, **kwargs
        )
        # update offsets
        ...
        map_projector = self.proj.clone(cam_pose=cam_pose, **kwargs)
        # Create top-down map
        topdown_map = TopdownMap(
            topdown_map=topdown_map,
            mask=mask,
    ...
    def merge(
        ...
        self._world_map = fuse_topdown_maps(
            self._world_map,
            topdown_map,
            map_projector=self.proj.clone(cam_pose=cam_pose),
            fill_value=fill_value,
            reduction=reduction,
        )
        return self._world_map
